...
| **Prospecção IA** | Agente autônomo (Claude, via Messages API + tool `web_search` server-side) que pesquisa a web semanalmente e alimenta `leads` com organizações candidatas a virar novos tenants — alimenta o mesmo funil comercial acima, nunca um CRM por organização. Config em `prospectingCampaigns/{campaignId}`, execuções auditadas em `prospectingRuns/{runId}`. |
| **Agente de Outbound** | Segundo agente de IA, extensão da Prospecção IA — gera abordagens comerciais personalizadas pra leads já qualificados, sempre com revisão humana antes de qualquer envio (nunca automático). Reaproveita o mesmo `ClaudeProvider` e o Lead existente. Estado em `outboundMessages/{leadId}` (versionado em `outboundMessages/{leadId}/versions`), lotes em `outboundBatches/{batchId}`, contexto comercial configurável em `systemConfig/salesContext`. |
