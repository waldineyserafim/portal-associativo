...
| 23 | **Nenhuma tela é acessível apenas por link direto sem também existir um caminho de navegação até ela** — exceto comprovantes/tokens de uso único (ex. `event_comprovante.html`), que são exceção documentada, não padrão. | IA, §2 | Telas "órfãs" (só alcançáveis por URL direta) quebram a sensação de mapa mental do produto — o usuário nunca sabe se "existe mais alguma coisa" que ele não está vendo. |
